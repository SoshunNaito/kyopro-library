...
public:
	...
	vector<pair<Cap,Cost>> min_cost_flow(int s, int t, Cap f) {
		...
		h.clear(); h.resize(V, 0);
		while (f > 0) {
			priority_queue<P, vector<P>, greater<P>> que;
			dist.clear(); dist.resize(V, 1 << 30);
			dist[s] = 0;
			que.push(P(0, s));
			while (que.size() > 0) {
				P p = que.top(); que.pop();
...
public:
	...
	struct edge { int from, to; Cap cap; Cost cost; };
	struct result { int from, to; Cap flow; Cost total; };
		...
		for (int i = 1; i < ans.size(); i++) {
			if (ans[i - 1].first <= F && F <= ans[i].first) {
				cout << ans[i - 1].second + (ans[i].second - ans[i - 1].second) *
					(F - ans[i - 1].first) / (ans[i].first - ans[i - 1].first) << endl;
				break;
			}
		}
	}
