...
inline Vec2 Projection(Vec2 P, Line L) {// éÀâe
	return P + L.norm * (L.dot - L.norm.dot(P));
}
inline Vec2 Intersection(Line L1, Line L2) {// 2íºê¸ÇÃåì_
	if (abs(L1.norm.cross(L2.norm)) < EPS) {// ï¿çsÇæÇ¡ÇΩèÍçá
...
inline Vec2 Centroid(Vec2 A, Vec2 B, Vec2 C) {// èdêS
	return (A + B + C) / 3;
}
inline Vec2 Centroid(Triangle T) {// èdêS
...
inline Circle Circumscribed_Circle(Triangle T) {// äOê⁄â~
...
inline Circle Inscribed_Circle(Vec2 A, Vec2 B, Vec2 C) {// ì‡ê⁄â~
	return Inscribed_Circle(Triangle(A, B, C));
}
inline Vec2 Orthocenter(Triangle T) {// êÇêS
	return Centroid(T) * 3 - Circumscribed_Circle(T).center * 2;
}
inline Vec2 Orthocenter(Vec2 A, Vec2 B, Vec2 C) {// êÇêS
	return Orthocenter(Triangle(A, B, C));
}
inline vector<Vec2> Intersection(Circle C, Line L) {// â~Ç∆íºê¸ÇÃåì_
...
inline vector<Vec2> Intersection(Circle C1, Circle C2) {// â~Ç∆â~ÇÃåì_
...
inline pair<Vec2*, int> convex_hull(vector<Vec2>& P) {// ì ïÔ
	int N = (int)P.size();
	vector<Vec2> v;
	{
...
Circle SmallestEnclosingDisk(vector<Vec2>& points) {// ç≈è¨ïÔä‹â~
