...
public:
	...
	inline T& at(int i, int j) {// 要素にアクセスする
		assert(i >= 0 && i < row && j >= 0 && j < column);
		return a[i * column + j];
	}
	inline MatrixClass Transpose() {// 転置
	...
	T determinant(bool sweep = false) {// sweep = true のときは掃き出し法、false のときは互除法で計算する
		assert(row == column);
	...
	void swap_row(int r1, int r2) {// 行の入れ替え
		r1 = r1 * column;
		r2 = r2 * column;
		T x;
	...
	void add_row(int r_src, int r_dest, T rate = 1) {// 行の定数倍を他の行に加える
		r_src *= column;
		r_dest *= column;
		for (int i = 0; i < column; i++) {
	...
	void swap_column(int c1, int c2) {// 列の入れ替え
		T x;
		for (int i = 0; i < row; i++) {
	...
	void add_column(int c_src, int c_dest, T rate = 1) {// 列の定数倍を他の行に加える
		for (int i = 0; i < row; i++) {
...
private:
	vector<T> Euclid(T a, T b) {// 互除法のサポート関数
...
template<typename T>
MatrixClass<T> Identity(int n) {// 単位行列
...
template<typename T>
MatrixClass<T> power(MatrixClass<T>& S, int n) {// 累乗
